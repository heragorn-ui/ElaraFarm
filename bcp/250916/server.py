# -*- coding: utf-8 -*-
# ElaraFarm Server v0.9.7 — SSE live + frame grid + resubmit + split-to-frames
...
app = FastAPI(title="ElaraFarm Server", version="0.9.7")
...
<form method="post" action="/submit_job" id="jobform">
  ...
  <fieldset><legend>Maya/Arnold Job</legend>
    ...
    <p class="small">Single machine best: <b>Chunk size = 0</b>, <b>By step = 1</b>.</p>
  ...
  const rows=data.map(g=>{
    ...
    const act = g.group_id
     ...
     : `<div style="display:flex;gap:6px;flex-wrap:wrap">
          ...
          <!-- Pause (graceful) and Stop now -->
          <button class="btn btn-ghost btn-sm" title="Pause after current frame"
                  onclick="doPost('/action/pause_job?id=${g.single_id}&mode=graceful')">Pause</button>
          <button class="btn btn-ghost btn-sm" title="Stop now"
                  onclick="doPost('/action/pause_job?id=${g.single_id}&mode=immediate')">Stop now</button>

          <button class="btn btn-ghost btn-sm" onclick="doPost('/action/resume_job?id=${g.single_id}')">Resume</button>

          <!-- Cancel after frame + now -->
          <button class="btn btn-ghost btn-sm" title="Cancel after current frame"
                  onclick="actConfirm('/action/cancel_job?id=${g.single_id}&mode=after_frame','Cancel after this frame?')">Cancel after frame</button>
          <button class="btn btn-danger btn-sm"
                  onclick="actConfirm('/action/cancel_job?id=${g.single_id}&mode=now','Cancel NOW?')">Cancel now</button>
...
def group_parts(gid:str="", job_id:int=0):
    ...
    for p in parts:
        ...
        out.append({"id":p.get("id"),"status":p.get("status") or "",
                    "start_frame":p.get("start_frame"),"end_frame":p.get("end_frame"),
                    "part_index":p.get("part_index"),"part_count":p.get("part_count"),
...
def delete_group(gid: str):
    c = db(); x = c.cursor()
    x.execute("SELECT COUNT(1) AS n FROM jobs WHERE group_id=? AND status='running'", (gid,))
    running = (x.fetchone() or {"n": 0})["n"]  # fixed bracket

    if running and running > 0:
        # mark as deleted; request cancel for running ones
